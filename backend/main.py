...
from backend.msg_push import (
    dingtalk, xizhi, tg_bot, send_email, bark, ntfy
)
from backend.ffmpeg_install import (
    check_ffmpeg, ffmpeg_path, current_env_path
)
from backend.redis_client import redis_client, get_client_tasks, set_client_tasks, delete_client_tasks
...
def init_error_url(client_id, url):
    """设置错误url"""
    error_url[f'{client_id}_{url}'] = 0

def get_error_url(client_id, url):
    """获取错误url"""
    return error_url[f'{client_id}_{url}'] if f'{client_id}_{url}' in error_url else 0

def add_error_url(client_id, url):
    """增加错误url"""
    if f'{client_id}_{url}' in error_url:
        error_url[f'{client_id}_{url}'] += 1 
    else:
        error_url[f'{client_id}_{url}'] = 1

def clear_error_url(client_id, url):
    """清除错误url"""
    if f'{client_id}_{url}' in error_url:
        del error_url[f'{client_id}_{url}']

def set_exit_flag(client_id, url):
    """设置退出标志"""
    exit_flag[f'{client_id}_{url}'] = threading.Event()

def get_exit_flag(client_id, url):
    """获取退出标志"""
    return exit_flag[f'{client_id}_{url}'].is_set() if f'{client_id}_{url}' in exit_flag else True

def clear_exit_flag(client_id, url):
    """清除退出标志"""
    exit_flag[f'{client_id}_{url}'].set()
    del exit_flag[f'{client_id}_{url}']

def set_ffmpeg_process(client_id, url, record_name, process):
    """设置ffmpeg进程"""
    ffmpeg_process[f'{client_id}_{url}'] = (process,record_name)

def clear_ffmpeg_process(client_id, url):
    """清除ffmpeg进程"""
    if f'{client_id}_{url}' in ffmpeg_process:
        print(f"---clear_ffmpeg_process: {client_id}_{url}")
        ffmpeg_process[f'{client_id}_{url}'][0].terminate()
        clear_record_info(ffmpeg_process[f'{client_id}_{url}'][1], url)
        del ffmpeg_process[f'{client_id}_{url}']
    
def stop_all_record(all_url_data: list, client_id: str):
    """停止所有录制录制"""
    # 获取该客户的url_comments
    comments = redis_client.get(f'comments:{client_id}')
    url_comments = loads(comments) if comments else []
    error8 = redis_client.get(f'error8:{client_id}')
    error8 = loads(error8) if error8 else []
    for url_data in all_url_data:
        if url_data[1] not in url_comments:
            url_comments.append(url_data[1])
        if url_data[1] in error8:
            error8.remove(url_data[1])
    # 保存回Redis
    redis_client.set(f'comments:{client_id}', dumps(url_comments))
    redis_client.set(f'error8:{client_id}', dumps(error8))

def stop_record(url_data: tuple, client_id: str):
    """停止录制"""
    # 获取该客户的url_comments
    comments = redis_client.get(f'comments:{client_id}')
    url_comments = loads(comments) if comments else []
...
def restart_stopped_record(url_data: tuple, client_id: str):
    """移除停止录制任务"""
    comments = redis_client.get(f'comments:{client_id}')
    url_comments = loads(comments) if comments else []
    if url_data[1] in url_comments:
        url_comments.remove(url_data[1])
        redis_client.set(f'comments:{client_id}', dumps(url_comments))
...
def get_port_info(url_data: tuple) -> list:
    ...
    if record_url.find("douyin.com/") > -1:
        ...
        with semaphore:
            if global_proxy or proxy_address:
                # 获得http请求返回的json数据
                # 如果输入的url错误，则返回的json中没有'LiveRoom'
                json_data = asyncio.run(spider.get_tiktok_stream_data(
                    url=record_url,
                    proxy_addr=proxy_address,
                    cookies=tiktok_cookie)) # 如何确定tiktok_cookie形式？多用户时tiktok_cookie需要修改
                # 获得json_data中流媒体播放地址
        ...
        return
    # ------------------区分不同平台结束------------------
    return port_info,platform

def get_task_info(task, client_id: str) -> dict:    
    try:
        port_info, platform = get_port_info(task['url_data'])
        if not port_info:  # 添加空值检查
            return {
                'anchor_name': '',
                'status': '录制出错',
                'platform': '',
        ...
        comments = redis_client.get(f'comments:{client_id}')
        url_comments = loads(comments) if comments else []
        if task['url_data'][1] in error8:
            status = "录制出错"
        elif task['url_data'][1] in url_comments:
            status = "停止录制"
        else:
            status = "直播中" if port_info.get("is_live", '') else "未开播"
    
        # 处理开始时间
        if 'status' in task and task['status'] == '未开播' and status == '直播中': # 从未开播-直播
            start_time = time.time()
        elif 'status' in task and task['status'] == '直播中' and status == '未开播': # 从直播-未开播
            start_time = task['start_time']
            status = "停止录制"
            url_comments.append(task['url_data'][1])
            redis_client.set(f'comments:{client_id}',dumps(url_comments))
        ...
        logger.error(f"get_task_info error: {e}")
        return {
            'anchor_name': '',
            'status': '录制出错',
            'platform': '',
...
def display_info(monitoring: int) -> None:
    ...
    while True:
        try:
            ...
            # print(f"目前瞬时错误数为: {get_error_url(client_id, url_data[1])}", end=" | ")
...
# 用于删除失效的直播URL、清理注释掉的配置、移除不需要监控的主播
...
def check_subprocess(record_name: str, record_url: str, ffmpeg_command: list, save_type: str,
                     client_id: str, script_command: str | None = None) -> bool:
    """
    执行和监控FFmpeg录制进程
    """
    ...
    set_ffmpeg_process(client_id, record_url,record_name, process)
    # 处理字幕文件路径
    ...
    if create_time_file and not split_video_by_time and '音频' not in save_type:
        ...
        create_var[subs_thread_name].daemon = True# 设为守护线程
        create_var[subs_thread_name].start()# 启动线程

    while process.poll() is None:# 当FFmpeg进程在运行时
        # 检查是否需要停止录制
        comments = redis_client.get(f'comments:{client_id}')
        url_comments = loads(comments) if comments else []
        if record_url in url_comments or exit_recording:
            ...
            if os.name == 'nt':# Windows
                ...
                clear_ffmpeg_process(client_id, record_url)
            process.wait()# 等待进程结束
            return True # 跳出check_subprocess，不会执行下面的代码
        time.sleep(1) # 每秒检查一次
...
def start_record(url_data: tuple, client_id: str, video_save_type: str, monitoring: int = 0):
    """开始录制"""
    init_error_url(client_id, url_data[1])
    while not get_exit_flag(client_id, url_data[1]): #外层循环：用于持续监控直播状态
        try:
            ...
            while not get_exit_flag(client_id, url_data[1]):  # 内层循环：持续录制直播流
                # 实际上好像不需要双层循环？
                try:
                    port_info = []
                    # ------------------区分不同平台------------------
                    if record_url.find("douyin.com/") > -1:
                        ...
                        with semaphore:
                            ...
                            if port_info and port_info.get('new_cookies'):
                                ...
                                logger.error("错误信息: 网络异常，请检查本网络是否能正常访问TwitCasting直播平台")
                    ...
                    if not port_info.get("anchor_name", ''):
                        print(f'序号{monitoring} 网址内容获取失败,进行重试中...获取失败的地址是:{url_data}')
                        with max_request_lock:
                            add_error_url(client_id, url_data[1])
                            error_window.append(1)
                    else:
                        # 更新redis数据
                        recording_tasks = get_client_tasks(client_id)
                        # 只在无数据或无anchor_name时更新任务列表
                        if url_data[1] not in recording_tasks:
                            if port_info['is_live']:
                                status = '直播中'
                                recording_tasks[url_data[1]] = {
                                    'url_data': url_data,
                                    'status': status,
                                    'anchor_name': anchor_name,
                                    'format': video_save_type,
                            ...
                            # print("更新redis数据2",recording_tasks)
                            set_client_tasks(client_id, recording_tasks)
                            recording_tasks = get_client_tasks(client_id)
                            # print("更新数据成功",recording_tasks)
                        elif recording_tasks[url_data[1]]['anchor_name'] == '':
                            if port_info['is_live']:
                                status = '直播中'
                            else:
                                status = '未开播'
                            ...
                            # print("3333platform",platform)
                            # print("更新redis数据3",recording_tasks)
                            set_client_tasks(client_id, recording_tasks)
                            recording_tasks = get_client_tasks(client_id)
                            # print("更新数据成功",recording_tasks)
                        anchor_name = clean_name(anchor_name) # 清理主播名中的特殊字符
                        record_name = f'序号{monitoring} {anchor_name}'
                        ...
                        if port_info['is_live'] is False:
                            print(f"\r{record_name} 等待直播...{datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S')}")

                            if start_pushed: # false
                                if over_show_push:
                        ...
                        # 如果监测正在直播，才会开始ffmpeg
                            ...
                            real_url = port_info.get('record_url') # 获得json_data中流媒体播放地址
                            full_path = f'{default_path}/{client_id}/{platform}'
                            if real_url:
                                ...
                                # 基本参数设置
                                rw_timeout = "15000000" # 读写超时时间
                                analyzeduration = "20000000" # 分析持续时间
                                probesize = "10000000"
                                bufsize = "8000k"
                                ...
                                ffmpeg_command = [
                                    'ffmpeg', "-y", # 覆盖已存在的文件
                                    "-v", "verbose", # 详细输出
                                    "-rw_timeout", rw_timeout, # 读写超时
                                    "-loglevel", "error", 
                                    "-hide_banner",
                                    "-user_agent", user_agent, # 设置用户代理
                                    ...
                                    "-sn", "-dn", # 不写入元数据
                                    "-reconnect_delay_max", "60", # 最大重连延迟
                                    "-reconnect_streamed", "-reconnect_at_eof", # 流中断时重连
                                    "-max_muxing_queue_size", max_muxing_queue_size, # 最大解复用队列大小
                                    "-correct_ts_overflow", "1", # 纠正时间戳溢出
                                ...
                                if video_save_type == "FLV" or only_flv_record:
                                    ...
                                    try:
                                        if split_video_by_time:
                                            ...
                                            save_file_path = f"{full_path}/{client_id}/{anchor_name}_{title_in_name}{now}_%03d.mkv"
                                            command = [
                                                "-flags", "global_header",
                                                "-c:v", "copy",
                                                "-c:a", "aac",
                ...
                if get_error_url(client_id, url_data[1]) > 20:
                    x = x + 60
                    color_obj.print_colored("\r瞬时错误太多,延迟加60秒", color_obj.YELLOW)# 增加60秒的延迟时间（x是一个延迟时间变量）
                ...
                # 这里是正常循环等待时间：x基本等于delay_default
                while x:
                    x = x - 1
                    if loop_time:
                        print(f'\r{anchor_name}循环等待{x}秒 ', end="")
                    if get_exit_flag(client_id, url_data[1]):
                        break
                    time.sleep(1)
                if loop_time:
                    print('\r检测直播间中...', end="")
...
def check_ffmpeg_existence() -> bool:
    try:
        # 通过 subprocess 模块调用 FFmpeg 命令行工具，即开始直播录制
...
# 处理客户在网页中输入的url
def url_init(url, quality):
    try:
        # 读取和处理URL配置文件
        if url == "www.tiktok.com" or url == "tiktok.com" or url == "https://www.tiktok.com" or url == "www.douyin.com" or url == "https://www.douyin.com" or url == "live.douyin.com/" or url == "https://live.douyin.com/":
            return None
        url = url.strip()
        # 清理app分享的链接
        if "https://v.douyin.com/" in url and "复制下方链接" in url:
            url = url.split("https://v.douyin.com/")[-1]
            url = "https://v.douyin.com/"+url
        elif "https://www.tiktok.com/" in url and "live?" in url:
            pattern = r"(https:\/\/www\.tiktok\.com\/@.+\/live\?)"
            try:    
                temp = re.search(pattern, url).group(1)
                if temp:
                    url = url[:-1]
            except Exception as e:
                return None
        elif "https://www.tiktok.com/@" in url and "?" in url:
        ...
        if url_host in platform_host or any(ext in url for ext in (".flv", ".m3u8")):
            ...
            new_line = (quality, url, '')
            # global url_tuples_list
            # url_tuples_list.append(new_line) # 有可能会有重复任务的
            # global text_no_repeat_url 
            # text_no_repeat_url = list(set(url_tuples_list))# 确保列表中不会有重复的直播任务
            return new_line
        # 如果上面的都不对，可能输入的是主播名
        else:
            if url.find("https://") > -1:
                ...
                if extra_enable_proxy_platform_list:
                    for pt in extra_enable_proxy_platform_list:
                        if pt and pt.strip() in url:
                            proxy_address = proxy_addr_bak or None
            if url.find("https://www.tiktok.com/") > -1:
                platform = 'TikTokLive'
                with semaphore:
                    if global_proxy or proxy_address:
                        # 获得http请求返回的json数据
...
def find_thread_by_name(name):
    for thread in threading.enumerate():
        if thread.name == name:
            return thread
    return None

def stop_thread_by_name(name,client_id,task):
    for thread in threading.enumerate():
        if thread.name == name:
            #  print(f"!!!stop_thread_by_name: {name}线程仍在运行")
            clear_exit_flag(client_id,task['url_data'][1])
            thread.join()
            # print(f"---stop_thread_by_name: {name}线程已停止")

def create_record_thread(url_tuple: tuple, client_id: str, format: str, monitoring: int = 0):
    """创建录制线程"""
    try:
        # 初始化退出标志
        set_exit_flag(client_id, url_tuple[1])
        print(f"---设置退出标志: exit_flag:{client_id}_{url_tuple[1]}")
        
        # 创建并启动线程
        create_var[f'thread_{monitoring}'] = threading.Thread(
            target=start_record, 
            args=[url_tuple, client_id, format, monitoring],
            name=f'{client_id}_{url_tuple[1]}'
        )
...
def init_config():
    ...
    if language and 'en' not in language.lower():
        from backend.i18n import translated_print
    ...
    try:
        ...
        if skip_proxy_check:
            ...
            print('系统代理检测中，请耐心等待...')
            response_g = urllib.request.urlopen("https://www.google.com/", timeout=10)
            print("---检测到代理",response_g)
            global_proxy = True
            print('\r全局/规则网络代理已开启√')
            pd = ProxyDetector()
...
def start_record_thread(url_tuple, client_id, format, monitoring):
    """多线程录制每个不重复的URL"""
    try:
        global first_start, first_run
        print(f"\r{'新增' if not first_start else '传入'}地址: {url_tuple[1]}")
                    # 创建新的录制线程
        args = [url_tuple, client_id, format, monitoring]  # 调整参数顺序
        # 对于每一个client_id+url，创建一个start_record线程
        set_exit_flag(client_id, url_tuple[1])
        create_var[f'thread_{monitoring}'] = threading.Thread(target=start_record, args=args, name=f'{client_id}_{url_tuple[1]}')
        create_var[f'thread_{monitoring}'].daemon = True
        create_var[f'thread_{monitoring}'].start()
        ...
        global url_tuples_list
        url_tuples_list = []
        return create_var[f'thread_{monitoring}']
